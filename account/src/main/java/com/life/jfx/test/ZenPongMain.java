...
public class ZenPongMain extends Application {
	...
	Timeline pongAnimation = TimelineBuilder
			.create()
			.keyFrames(
					new KeyFrame(new Duration(10.0),
							new EventHandler<ActionEvent>() {
								public void handle(javafx.event.ActionEvent t) {
									checkForCollision();
									int horzPixels = movingRight ? 1 : -1;
									int vertPixels = movingDown ? 1 : -1;
									centerX.setValue(centerX.getValue()
											+ horzPixels);
									centerY.setValue(centerY.getValue()
											+ vertPixels);
								}
		...
		Scene scene = SceneBuilder
				...
				.fill(LinearGradientBuilder
						...
						.stops(new Stop(0.0, Color.BLACK),
								new Stop(0.0, Color.GRAY)).build())
				.root(pongComponents = GroupBuilder
						.create()
						.focusTraversable(true)
						.children(
								ball = CircleBuilder.create().radius(5.0)
										.fill(Color.WHITE).build(),
								topWall = RectangleBuilder.create().x(0).y(0)
										.width(500).height(1).build(),
								leftWall = RectangleBuilder.create().x(0).y(0)
										.width(1).height(500).build(),
								rightWall = RectangleBuilder.create().x(500)
										.y(0).width(1).height(500).build(),
								bottomWall = RectangleBuilder.create().x(0)
										.y(500).width(500).height(1).build(),
								leftPaddle = RectangleBuilder
										.create()
										.x(20)
										.width(10)
										.height(30)
										...
										.onMouseDragged(
												new EventHandler<MouseEvent>() {
													public void handle(
														...
														leftPaddleY
																.setValue(initLeftPaddleTranslateY
																		+ dragY);
													}
												}).build(),
								rightPaddle = RectangleBuilder
										.create()
										.x(470)
										.width(10)
										.height(30)
										...
										.onMouseDragged(
												new EventHandler<MouseEvent>() {
													public void handle(
														...
														rightPaddleY
																.setValue(initRightPaddleTranslateY
																		+ dragY);
													}
												}).build(),
						...
						.onKeyPressed(new EventHandler<KeyEvent>() {
							...
							public void handle(KeyEvent k) {
								if (k.getCode() == KeyCode.SPACE
									...
									rightPaddleY.setValue(rightPaddleY
											.getValue() - 6);
